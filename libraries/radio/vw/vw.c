...
bool TIMER1_COMPA_vect(struct repeating_timer *t);
    ...
    if (vw_rx_pll_ramp >= VW_RX_RAMP_LEN)
    {
        ...
        if (vw_rx_active)
        {
            ...
            if (++vw_rx_bit_count >= 12)
            {
                ...
                uint8_t this_byte =
                    (vw_symbol_6to4(vw_rx_bits & 0x3f)) << 4 | vw_symbol_6to4(vw_rx_bits >> 6);
...
static uint8_t _timer_calc(uint16_t speed, uint16_t max_ticks, uint16_t *nticks)
{
    ...
    for (prescaler = 1; prescaler < 7; prescaler += 1)
    {
        ...
        float clock_time = (1.0f / ((float)F_CPU / (float)prescalers[prescaler]));
        // Fraction of second needed to xmit one bit
        float bit_time = ((1.0f / (float)speed) / 8.0f);
...
{
    // Create Interrupt/Repeating Timer
    struct repeating_timer timer;
    add_repeating_timer_us(-1000000 / speed, TIMER1_COMPA_vect, NULL, &timer);

    // Set up digital IO pins
    gpio_init(vw_tx_pin);
    gpio_init(vw_rx_pin);
    gpio_init(vw_ptt_pin);

    gpio_set_dir(vw_tx_pin, GPIO_OUT);
    gpio_set_dir(vw_rx_pin, GPIO_IN);
    gpio_set_dir(vw_ptt_pin, GPIO_OUT);

    gpio_put(vw_ptt_pin, vw_ptt_inverted);
}
...
void vw_tx_start()
{
    ...
    gpio_put(vw_ptt_pin, true ^ vw_ptt_inverted);
...
void vw_tx_stop()
{
    ...
    gpio_put(vw_ptt_pin, false ^ vw_ptt_inverted);
    gpio_put(vw_tx_pin, false);
...
bool TIMER1_COMPA_vect(struct repeating_timer *t)
{
    if (vw_rx_enabled && !vw_tx_enabled)
        vw_rx_sample = gpio_get(vw_rx_pin);
    ...
    if (vw_tx_enabled && vw_tx_sample++ == 0)
    {
        ...
        {
            gpio_put(vw_tx_pin, vw_tx_buf[vw_tx_index] & (1 << vw_tx_bit++));
            if (vw_tx_bit >= 6)
            {
                vw_tx_bit = 0;
                vw_tx_index++;
